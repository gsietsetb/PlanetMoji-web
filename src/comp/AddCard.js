...
import {bordColor, colors, isWeb, textSize} from '../gStyles';
...
export default ({item, list = unitsMap, border = colors.blue, own = false, currLife, onSet = profile.buyUnit}) => {
  ...
  return (
    <View style={apply(C.m1, own ? C.p2 : C.p1, C.bgWhite, C.radius2, bordColor(border, 2))}>
      <TouchableOpacity
        activeOpacity={0.4}
        ...
        {currLife && !isWeb ? (
          <TrackBar progress={currLife / skills['❤️']} />
        ) : (
